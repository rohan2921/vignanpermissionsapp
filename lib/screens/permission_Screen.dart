    ...
    return Scaffold(
        ...
        body: Column(
          children: <Widget>[
            Expanded(
                          child: StreamBuilder(
                stream:  Firestore.instance.collection('permissions').snapshots(), 
                builder: (ctx,snaps){
                  ...
                  return ListView.builder(itemBuilder: (ctx,ind){
                        return ListTile(
                          leading: Text(perms[ind]['topic'],style:TextStyle(color:perms[ind]['read']?Colors.blue[300]
                          : Colors.red[200]) ,),
                          onTap: () => Navigator.of(context).popAndPushNamed(GrantPermission.routeName,arguments: perms[ind]),
                        );
